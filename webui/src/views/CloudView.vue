...
function hasCredentials(connection: Connection) {
  return connection.kind === 'webdav' || connection.has_token || (connection.has_username && connection.has_password)
}
...
<template>
  ...
  <section class="section-block"><div class="table-wrap"><table><thead><tr><th>{{ t('Name') }}</th><th>{{ t('Type') }}</th><th>{{ t('Endpoint') }}</th><th>{{ t('Credentials') }}</th><th><span class="sr-only">{{ t('Actions') }}</span></th></tr></thead><tbody>
