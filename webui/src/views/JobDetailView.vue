...
import { api, errorMessage, type Job, type JobArtifact, type JobLog } from '../api'
...
async function load(changedId = false) {
  ...
  try {
    const [result, logResult] = await Promise.all([
      api.job(id),
      api.jobLogs(id, changedId ? undefined : logs.value[logs.value.length - 1]?.id),
    ])
    if (version !== loadVersion) return
    job.value = result
    logs.value = changedId ? logResult.logs : [...logs.value, ...logResult.logs]
    error.value = ''
  } catch (reason) {
    if (version !== loadVersion) return
    job.value = null
...
async function cancel() {
  if (!job.value || !window.confirm(t(job.value.state === 'running' ? 'Request cancellation for this running job?' : 'Cancel this queued job?'))) return
  const version = loadVersion
  actionError.value = ''
  try {
...
<template>
  ...
  <template v-if="job">
    <section class="detail-grid"><article class="section-block"><div class="section-heading"><div><p class="eyebrow">{{ t('Request') }}</p><h2>{{ t('Parameters') }}</h2></div></div><dl class="facts"><div><dt>{{ t('Type') }}</dt><dd>{{ valueLabel(job.kind) }}</dd></div><div><dt>{{ t('Origin') }}</dt><dd>{{ valueLabel(job.origin) }}</dd></div><div><dt>{{ t('Attempts') }}</dt><dd>{{ job.attempts }}</dd></div><div><dt>{{ t('Created') }}</dt><dd>{{ formatDateTime(job.created_at) }}</dd></div><div v-if="job.started_at"><dt>{{ t('Started') }}</dt><dd>{{ formatDateTime(job.started_at) }}</dd></div><div v-if="job.finished_at"><dt>{{ t('Finished') }}</dt><dd>{{ formatDateTime(job.finished_at) }}</dd></div><div><dt>{{ t('Duration') }}</dt><dd>{{ formatDuration(job.started_at, job.finished_at) }}</dd></div></dl><pre class="json-block">{{ JSON.stringify(job.request, null, 2) }}</pre></article><article class="section-block"><div class="section-heading"><div><p class="eyebrow">{{ t('Outcome') }}</p><h2>{{ t('Result') }}</h2></div></div><p v-if="job.progress_message" class="progress-message">{{ job.progress_message }}</p><p v-if="job.state === 'running' && job.cancelable" class="notice">{{ t(job.cancel_requested ? 'Cancellation requested; waiting for a safe checkpoint.' : 'Running jobs stop after the current safe operation.') }}</p><p v-if="job.error" class="alert error">{{ job.error }}</p><pre v-if="job.result" class="json-block">{{ JSON.stringify(job.result, null, 2) }}</pre><div v-if="artifactsFor(job).length" class="artifact-list"><span class="label">{{ t('Artifacts') }}</span><a v-for="artifact in artifactsFor(job)" :key="artifact.id" class="download-link" :href="artifact.download_url" :download="artifact.name">{{ artifact.name }} ({{ t('{size} bytes', { size: artifact.size }) }})</a></div><p v-if="!job.result && !job.error" class="empty-state">{{ t('No final result yet.') }}</p></article></section>
    <section class="section-block log-section" aria-live="polite"><div class="section-heading"><div><p class="eyebrow">{{ t('Logs') }}</p><h2>{{ t('Live execution log') }}</h2></div></div><div class="job-log" role="log"><div v-for="entry in logs" :key="entry.id" class="job-log-line" :class="entry.level"><time :datetime="entry.created_at">{{ formatDateTime(entry.created_at) }}</time><span>{{ entry.message }}</span></div><p v-if="!logs.length" class="empty-state">{{ t('No log entries yet.') }}</p></div></section>
  </template>
</template>
