...
describe('RSS and CloudDrive API contracts', () => {
  it('posts typed RSS subscription fields with the default organization interval without browser persistence', async () => {
    ...
    expect(JSON.parse(fetchMock.mock.calls[0][1].body)).toMatchObject({ url: 'https://example.test/rss', connection_id: 4, organize_interval_secs: 300 })
  ...
  it('posts a separate organization interval for automatic CloudDrive organization', async () => {
    const fetchMock = vi.fn().mockResolvedValue(new Response(JSON.stringify({ id: 1 }), { status: 201 }))
    vi.stubGlobal('fetch', fetchMock)
    await api.createSubscription({ url: 'https://example.test/rss', target_folder: '/incoming', interval_secs: 900, connection_id: 4, auto_organize: true, organize_target_folder: '/anime', organize_interval_secs: 600, organize_season_mode: true, remove_empty_dirs: false })
    expect(JSON.parse(fetchMock.mock.calls[0][1].body)).toMatchObject({ interval_secs: 900, auto_organize: true, organize_target_folder: '/anime', organize_interval_secs: 600 })
  })
